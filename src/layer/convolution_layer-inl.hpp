...
class ConvolutionLayer : public ILayer<xpu> {
 public:
  ...
  virtual void InitConnection(const std::vector<Node<xpu>*> &nodes_in,
    ...
    utils::Check(ishape[2] % param_.num_group == 0,  "input channels must divide group size");
    ...
    mshadow::Shape<4> oshape = mshadow::
        Shape4(ishape[3], param_.num_channel,
                (ishape[1] + 2 * param_.pad_y - ksize_y) / kstride + 1,
                (ishape[0] + 2 * param_.pad_x - ksize_x) / kstride + 1);
    nodes_out[0]->data.shape = oshape;

    if (param_.num_input_channel == 0) {
      param_.num_input_channel = static_cast<int>(ishape[2]);
    } else {
      utils::Check(param_.num_input_channel == static_cast<int>(ishape[2]),
  ...
  virtual void Forward(bool is_train,
    ...
    this->InitTemp(in.shape, out.shape);
 ...
 private:
  inline void InitTemp(mshadow::Shape<4> ishape, mshadow::Shape<4> oshape) {
    ...
    shape_colunit_ = mshadow::Shape2(ishape[2] * ksize_y * ksize_x, oshape[1] * oshape[0]);
    shape_dstunit_ = mshadow::Shape3(param_.num_group, param_.num_channel/param_.num_group, oshape[1]*oshape[0]);
    nstep_ = std::max(std::min((index_t)(param_.temp_col_max / shape_colunit_.Size()), ishape[3]), 1U);
    // make nstep more balanced,  nstep will use exactly same number of operations to finish,
    index_t nop = (ishape[3]+nstep_-1) / nstep_;
    nstep_ = (ishape[3] + nop - 1)/ nop;
